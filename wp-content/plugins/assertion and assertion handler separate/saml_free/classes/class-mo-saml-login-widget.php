...
function mo_saml_login_validate() {
	...
	if ( isset( $_REQUEST['option'] ) && 'mo_fix_certificate' === $_REQUEST['option'] && is_user_logged_in() && current_user_can( 'manage_options' ) ) {
		...
		$saml_certificate[0]       = Mo_SAML_SSO_Utilities::mo_saml_sanitize_certificate( $saml_required_certificate );
	...
	if ( ! empty( $_POST['SAMLResponse'] ) ) {
		...
		if ( is_array( $cert_from_plugin ) ) {
			...
			$cert_fp_from_plugin = preg_replace( '/\s+/', '', $cert_fp_from_plugin );
			if ( ! empty( $response_signature_data ) ) {
				$valid_signature = Mo_SAML_SSO_Utilities::mo_saml_process_response( $acs_url, $cert_fp_from_plugin, $response_signature_data, $saml_response, 0, $relay_state );
			}

			if ( ! empty( $assertion_signature_data ) ) {
				$valid_signature = Mo_SAML_SSO_Utilities::mo_saml_process_response( $acs_url, $cert_fp_from_plugin, $assertion_signature_data, $saml_response, 0, $relay_state );
			}
		}
		...
		if ( ! $valid_signature ) {
			...
			$desanitized_certificate = Mo_SAML_SSO_Utilities::mo_saml_desanitize_certificate( $plugin_cert );
