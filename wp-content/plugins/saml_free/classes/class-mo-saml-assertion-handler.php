...
class Mo_SAML_Assertion_Handler
{
    ...
    private static $assertion= null;

    public function __construct($mo_saml_assertion,$xml){
        $this->assertion = $mo_saml_assertion;
     ...
     */
    public static function getInstance($mo_saml_assertion,$param = null): Mo_SAML_Assertion_Handler
    {
 
        if (self::$instance === null) {
                self::$instance = new self($mo_saml_assertion,$param);
        }
   ...
   /**
    ...
    private function mo_saml_parse_conditions(DOMElement $xml)
    {
        ...
        if ($conditions->hasAttribute('NotBefore')) {
            ...
            if (null === $this->assertion->mo_saml_get_not_before() || $this->assertion->mo_saml_get_not_before() < $not_before) {
       $this->assertion->mo_saml_set_not_before($not_before);
            }
        }
        if ($conditions->hasAttribute('NotOnOrAfter')) {
            $not_on_or_after = Mo_SAML_Utilities::mo_saml_xs_date_time_to_timestamp($conditions->getAttribute('NotOnOrAfter'));
            if (null === $this->assertion->mo_saml_get_not_on_or_after() || $this->assertion->mo_saml_get_not_on_or_after() > $not_on_or_after) {
                $this->assertion->mo_saml_set_not_on_or_after($not_on_or_after);
            }
        }
        ...
        for ($node = $conditions->firstChild; null !== $node; $node = $node->nextSibling) {
            ...
            switch ($node->localName) {
                case 'AudienceRestriction':
                    ...
                    if (null === $this->assertion->mo_saml_get_valid_audiences()) {
                        /* The first (and probably last) AudienceRestriction element. */
                        $this->assertion->mo_saml_set_valid_audiences($audiences);
                    } else {
                        /*
                         * The set of AudienceRestriction are ANDed together, so we need
                         * the subset that are present in all of them.
                         */
                        $this->assertion->mo_saml_set_valid_audiences(array_intersect($this->assertion->mo_saml_get_valid_audiences(), $audiences));
                    }
                    break;
                case 'OneTimeUse':
                    /* Currently ignored. */
   ...
   /**
    ...
    { 
        ...
        if (empty($authn_statements)) {
            $this->assertion->mo_saml_set_authn_instant(null);
        ...
        $this->assertion->mo_saml_set_authn_instant(Mo_SAML_Utilities::mo_saml_xs_date_time_to_timestamp($authn_statement->getAttribute('AuthnInstant')));

        if ($authn_statement->hasAttribute('SessionNotOnOrAfter')) {
            $this->assertion->mo_saml_set_session_not_on_or_after(Mo_SAML_Utilities::mo_saml_xs_date_time_to_timestamp($authn_statement->getAttribute('SessionNotOnOrAfter')));
        }

        if ($authn_statement->hasAttribute('SessionIndex')) {
            $this->assertion->mo_saml_set_session_index($authn_statement->getAttribute('SessionIndex'));
        }
    ...
    private function mo_saml_parse_subject(DOMElement $xml)
    {
        ...
        if ('EncryptedData' === $name_id->localName) {
            ...
            $this->assertion->mo_saml_set_encrypted_name_id($name_id);
        } else {
            // $this->name_id = Mo_SAML_Utilities::mo_saml_parse_name_id($name_id);
            $this->assertion->mo_saml_set_name_id(Mo_SAML_Utilities::mo_saml_parse_name_id($name_id));
        }
    }
    ...
    {  
        ...
        if (count($authn_context_decl_refs) > 1) {
            ...
            $this->assertion->mo_saml_set_authn_context_decl_ref(trim($authn_context_decl_refs[0]->textContent));
        }
        ...
        if (count($authn_context_decls) > 1) {
            ...
            $this->assertion->mo_saml_set_authn_context_decl(new SAML2_XML_Chunk($authn_context_decls[0]));
        }
        ...
        if (count($authn_context_class_refs) > 1) {
            ...
            $this->assertion->mo_saml_set_authn_context_class_ref(trim($authn_context_class_refs[0]->textContent));
        }

        // Constraint from XSD: MUST have one of the three.
        if (empty($this->assertion->mo_saml_get_authn_context()) && empty($this->assertion->mo_saml_get_authn_aontext_aecl()) && empty($this->assertion->mo_saml_get_authn_context_decl_ref())) {
            throw new Exception('Missing either <saml:AuthnContextClassRef> or <saml:AuthnContextDeclRef> or <saml:AuthnContextDecl>');
        }

        $this->assertion->mo_saml_set_authenticating_authority(Mo_SAML_SSO_Utilities::mo_saml_extract_strings($authn_context_el, 'urn:oasis:names:tc:SAML:2.0:assertion', 'AuthenticatingAuthority'));
    }
    ...
    {  
        ...
        foreach ($attributes as $attribute) {
            ...
            if ($first_attribute) {
                ...
                $this->assertion->set_attribute_name_format($name_format);
                $first_attribute = false;
            } else {
                if ($this->assertion->mo_saml_get_attribute_name_format() !== $name_format) {
                    $this->assertion->set_attribute_name_format('urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified');
                }
            }
            $instance_attributes = $this->assertion->mo_saml_get_attributes();
    ...
    {   $this->assertion->mo_saml_set_encrypted_attribute(Mo_SAML_SSO_Utilities::mo_saml_xp_query($xml, './saml_assertion:AttributeStatement/saml_assertion:EncryptedAttribute'));
    }
    ...
    { 
        ...
        if (false !== $sig) {
            $this->assertion->mo_saml_set_was_signed_at_construction(true);
            $this->assertion->mo_saml_set_certificates($sig['Certificates']);
            $this->assertion->mo_saml_set_signature_data($sig);
        }
    }
    ...
    {
        if (null === $this->assertion->mo_saml_get_signature_data()) {
            return false;
        }

        Mo_SAML_SSO_Utilities::mo_saml_validate_signature($this->assertion->mo_saml_get_signature_data(), $key);
    ...
    public function mo_saml_encrypt_name_id(Mo_SAML_XML_Security_Key $key)
    {
        ...
        Mo_SAML_Utilities::addNameId($root, $this->assertion->mo_saml_get_name_id());
        ...
        $this->assertion->mo_saml_set_encrypted_name_id($enc->encryptNode($symmetric_key));
        $this->assertion->mo_saml_set_name_id(null);
    }
    ...
    public function mo_saml_to_xml(DOMNode $parent_element = null)
    {
        ...
        $root->setAttribute('IssueInstant', gmdate('Y-m-d\TH:i:s\Z', $this->assertion->mo_saml_get_issue_instant()));

        $issuer = Mo_SAML_Utilities::addString($root, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Issuer', $this->assertion->mo_saml_get_issuer());
        ...
        if (null !== $this->assertion->mo_saml_get_signature_key()) {
            // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- Working with PHP DOMDocument Attributes.
            Mo_SAML_Utilities::insertSignature($this->assertion->mo_saml_get_signature_key(), $this->assertion->mo_saml_get_certificates(), $root, $issuer->nextSibling);
        }
    ...
    {
        if (null === $this->assertion->mo_saml_get_name_id() && null === $this->assertion->mo_saml_get_encrypted_name_id()) {
        ...
        if (null === $this->assertion->mo_saml_get_encrypted_name_id()) {
            Mo_SAML_Utilities::addNameId($subject, $this->assertion->mo_saml_get_name_id());
        } else {
            // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- Working with PHP DOMElement Attributes.
            $eid = $subject->ownerDocument->createElementNS('urn:oasis:names:tc:SAML:2.0:assertion', 'saml:EncryptedID');
            $subject->appendChild($eid);
            // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- Working with PHP DOMElement Attributes.
            $eid->appendChild($subject->ownerDocument->importNode($this->assertion->mo_saml_get_encrypted_name_id(), true));
        }

        foreach ($this->assertion->mo_saml_get_subject_confirmation() as $sc) {
    ...
    private function mo_saml_add_conditions(DOMElement $root)
    {
        ...
        if (null !== $this->assertion->mo_saml_get_not_before()) {
            $conditions->setAttribute('NotBefore', gmdate('Y-m-d\TH:i:s\Z', $this->assertion->mo_saml_get_not_before()));
        }
        if (null !== $this->assertion->mo_saml_get_not_on_or_after()) {
            $conditions->setAttribute('NotOnOrAfter', gmdate('Y-m-d\TH:i:s\Z', $this->assertion->mo_saml_get_not_on_or_after()));
        }
        ...
        if (null !== $this->assertion->mo_saml_get_valid_audiences()) {
            ...
            Mo_SAML_Utilities::addStrings($ar, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:Audience', false, $this->assertion->mo_saml_get_valid_audiences());
        }
    }
    ...
    {
        if (null === $this->assertion->mo_saml_get_authn_instant() || (null === $this->assertion->mo_saml_get_authn_context_class_ref() && null === $this->assertion->mo_saml_get_authn_aontext_aecl() && null === $this->assertion->mo_saml_get_authn_context_decl_ref())) {
        ...
        $authn_statement_el->setAttribute('AuthnInstant', gmdate('Y-m-d\TH:i:s\Z', $this->assertion->mo_saml_get_authn_instant()));

        if (null !== $this->assertion->mo_saml_get_session_not_on_or_after()) {
            $authn_statement_el->setAttribute('SessionNotOnOrAfter', gmdate('Y-m-d\TH:i:s\Z', $this->assertion->mo_saml_get_session_not_on_or_after()));
        }
        if (null !== $this->assertion->mo_saml_get_session_index()) {
            $authn_statement_el->setAttribute('SessionIndex', $this->assertion->mo_saml_get_session_index());
        }
        ...
        if (! empty($this->assertion->mo_saml_get_authn_context_class_ref())) {
            Mo_SAML_Utilities::addString($authn_context_el, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:AuthnContextClassRef', $this->assertion->mo_saml_get_authn_context_class_ref());
        }
        if (! empty($this->assertion->mo_saml_get_authn_aontext_aecl())) {
            $this->assertion->mo_saml_get_authn_aontext_aecl()->mo_saml_to_xml($authn_context_el);
        }
        if (! empty($this->assertion->mo_saml_get_authn_context_decl_ref())) {
            Mo_SAML_Utilities::addString($authn_context_el, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:AuthnContextDeclRef', $this->assertion->mo_saml_get_authn_context_decl_ref());
        }

        Mo_SAML_Utilities::addStrings($authn_context_el, 'urn:oasis:names:tc:SAML:2.0:assertion', 'saml:AuthenticatingAuthority', false, $this->assertion->mo_saml_get_authenticating_authority());
    }
    ...
    {
        if (empty($this->assertion->mo_saml_get_attributes())) {
            return;
        }
        // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase -- Working with PHP DOMElement Attributes.
        $document = $root->ownerDocument;
        ...
        foreach ($this->assertion->mo_saml_get_attributes() as $name => $values) {
            ...
            if ('urn:oasis:names:tc:SAML:2.0:attrname-format:unspecified' !== $this->assertion->mo_saml_get_attribute_name_format()) {
                $attribute->setAttribute('NameFormat', $this->assertion->mo_saml_get_attribute_name_format());
            }
...
//test
        ...
        foreach ($this->assertion->mo_saml_get_attributes() as $name => $values) {
            ...
            $symmetric_key->mo_saml_generate_session_key();
            $enc_assert->encryptKey($this->assertion->mo_saml_get_encryption_key(), $symmetric_key);
