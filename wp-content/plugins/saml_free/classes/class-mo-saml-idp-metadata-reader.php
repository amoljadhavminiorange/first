...
class Mo_SAML_IDP_Metadata_Reader {
	...
	public function __construct( DOMNode $xml = null ) {
		...
		$entities_descriptor = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:EntitiesDescriptor' );

		if ( ! empty( $entities_descriptor ) ) {
			$entity_descriptors = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $entities_descriptor[0], './saml_metadata:EntityDescriptor' );
		} else {
			$entity_descriptors = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:EntityDescriptor' );
		}

		foreach ( $entity_descriptors as $entity_descriptor ) {
			$idp_sso_escriptor = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $entity_descriptor, './saml_metadata:IDPSSODescriptor' );
...
class Mo_SAML_Identity_Providers {
	...
	public function __construct( DOMElement $xml = null ) {
		...
		$idp_sso_descriptor = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:IDPSSODescriptor' );
		...
		$info = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:Extensions' );
	...
	private function mo_saml_parse_info( $xml ) {
		$display_names = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './mdui:UIInfo/mdui:DisplayName' );
	...
	private function mo_saml_parse_sso_service( $xml ) {
		$sso_services = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:SingleSignOnService' );
	...
	private function mo_saml_parse_slo_service( $xml ) {
		$slo_services = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:SingleLogoutService' );
	...
	private function mo_saml_parsex509_certificate( $xml ) {
		foreach ( Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './saml_metadata:KeyDescriptor' ) as $key_descriptor_node ) {
	...
	private function mo_saml_parse_signing_certificate( $xml ) {
		$cert_node = Mo_SAML_SSO_Utilities::mo_saml_xp_query( $xml, './ds:KeyInfo/ds:X509Data/ds:X509Certificate' );
		$cert_data = trim( $cert_node[0]->textContent );
		$cert_data = str_replace( array( "\r", "\n", "\t", ' ' ), '', $cert_data );
		if ( ! empty( $cert_node ) ) {
			array_push( $this->signing_certificate, Mo_SAML_SSO_Utilities::mo_saml_sanitize_certificate( $cert_data ) );
		}
	}
