...
class Mo_SAML_Login_Widget extends WP_Widget
{
    ...
    {
        parent::__construct('Saml_Login_Widget', 'Login with ' . esc_html(get_option(Mo_Saml_Options_Enum_Service_Provider::IDENTITY_NAME)), array(
     ...
     *
     * @param array $args
     *            Display arguments including 'before_title', 'after_title',
...
<form name="miniorange-saml-sp-sso-login-form"
	id="miniorange-saml-sp-sso-login-form" method="post" action="">
	<input type="hidden" name="option" value="saml_user_login" /> <font
		size="+1" style="vertical-align: top;"> </font>
				<?php
            ...
            if (! Mo_SAML_Utilities::mo_saml_check_empty_or_null(array(
                get_option(Mo_Saml_Sso_Constants::MO_SAML_REDIRECT_ERROR)
            ))) {
...
<?php
			...
			|
<a
	href="<?php echo esc_url( wp_logout_url( mo_saml_get_current_page_url() ) ); ?>"
	title="<?php esc_attr_e( 'Logout', 'miniorange-saml-20-single-sign-on' ); ?>"><?php esc_html_e( 'Logout', 'miniorange-saml-20-single-sign-on' ); ?></a>
</li>
<?php
        }
    }
...
function mo_saml_login_validate()
{
    ...
    if ((isset($_REQUEST['option']) && 'saml_user_login' === $_REQUEST['option']) || (isset($_REQUEST['option']) && 'testConfig' === $_REQUEST['option'])) {
        ...
        if (Mo_SAML_Utilities::mo_saml_is_sp_configured()) {
            ...
            $saml_request = Mo_SAML_SSO_Utilities::mo_saml_create_authn_request($acs_url, $sp_entity_id);
            ...
            Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_RELAYSTATE_SENT', array(
                'redirect' => $redirect
    ...
    if (! empty($_POST['SAMLResponse'])) {
        ...
        Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_SAML_RESPONSE', array(
        ...
        Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_RELAYSTATE_RECEIVED', array(
        ...
        if (is_array($cert_from_plugin)) {
            foreach ($cert_from_plugin as $key => $value) {
                ...
                if (! empty($response_signature_data)) {
                    $valid_signature = Mo_SAML_SSO_Utilities::mo_saml_process_response($acs_url, $cert_fp_from_plugin, $response_signature_data, $saml_response, $key, $relay_state);
                }
                if (! empty($assertion_signature_data)) {
                    $valid_signature = Mo_SAML_SSO_Utilities::mo_saml_process_response($acs_url, $cert_fp_from_plugin, $assertion_signature_data, $saml_response, $key, $relay_state);
                }
                if ($valid_signature) {
                    break;
                }
        ...
        Mo_SAML_SSO_Utilities::mo_saml_validate_issuer_and_audience($saml_response, $sp_enity_id, $issuer, $relay_state);
        ...
        $attrs['NameID'] = array(
            '0' => sanitize_text_field($ssoemail)
        );
        $session_index = current($saml_response->mo_saml_get_assertions())->mo_saml_get_session_index();
        Mo_SAML_Logger::mo_saml_add_log(mo_saml_error_log::mo_saml_write_message('ATTRIBUTES_RECEIVED_IN_TEST_CONFIGURATION', array(
            'attrs' => $attrs
        )), Mo_SAML_Logger::INFO);
 ...
 *
 * @param array $attrs
 *            array of attribute node of SAML response.
 *            
 * @param string $relay_state
 *            redirect url for post SSO flow.
 *            
 * @param string $session_index
 *            session index to after authentication.
 *            
 * @return void
 */
 ...
 *            first name of user from the SAML response.
 *            
 * @param string $last_name
 *            last name of user from the SAML response.
 *            
 * @param string $user_email
 *            email of user from the SAML response.
 *            
 * @param array|string $group_name
 *            mapped group name of user from the SAML response.
 *            
 * @param array $attrs
 *            array of attributes received in the attributes node of the SAML response.
 *            
 * @return void
 */
 ...
 *
 * @param string $cert_fp_from_plugin
 *            IdP certificate from the plugin.
 * @return string
 */
 ...
 *
 * @param string $user_email
 *            email for user from the SAML response ( NameID or Subject Node in the SAML response).
 *            
 * @param string $first_name
 *            first name of user from SAML response.
 *            
 * @param string $last_name
 *            last name of user from SAML response.
 *            
 * @param string $user_name
 *            user name of user from SAML response.
 *            
 * @param array|string $group_name
 *            group name of user from SAML response.
 *            
 * @param string $default_role
 *            default role from the plugin configurations saved in DB.
 *            
 * @param string $relay_state
 *            relay state parameter passed by IDP.
 *            
 * @param string $check_if_match_by
 *            default username, parameter from which users will be matched.
 *            
 * @param string $session_index
 *            session index sent by IDP.
 *            
 * @param string $name_id
 *            NameID or subject node from the SAML response.
 *            
 * @return void
 */
function mo_saml_login_user($user_email, $first_name, $last_name, $user_name, $group_name, $default_role, $relay_state, $check_if_match_by, $session_index = '', $name_id = '')
{
    ...
    if (('username' === $check_if_match_by && username_exists($user_name)) || username_exists($user_name)) {
        ...
        Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_USER_EXISTS', array(
            'userName' => $user_name
        )), Mo_SAML_Logger::DEBUG);
        ...
        Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_EMAIL_EXISTS', array(
        ...
        Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_NEW_USER', array(
            'user_email' => $user_email,
            'user_id' => $user_id
        )), Mo_SAML_Logger::DEBUG);
        ...
        if (! empty($default_role) && ! mo_saml_is_administrator_user(get_user_by('id', $user_id))) {
            ...
            Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_DEFAULT_ROLE', array(
 ...
 *            
 * @param string $relay_state
 *            relay state parameter. URL where the user should be redirected to after authentication.
 * @return void
 */
function mo_saml_add_firstlast_name($user_id, $first_name, $last_name, $relay_state)
{
    ...
    Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_COOKIE_CREATED', array(
        'user_id' => $user_id
    )), Mo_SAML_Logger::DEBUG);
    ...
    Mo_SAML_Logger::mo_saml_add_log(Mo_Saml_Error_Log::mo_saml_write_message('LOGIN_WIDGET_REDIRECT_URL_AFTER_LOGIN', array(
 ...
 *            status code from the SAML response.
 *            
 * @param string $relay_state
 *            relay state parameter from the SAML response.
 *            
 * @param string $statusmessage
 *            status message returned from the IDP in the SAML response.
 *            
 * @return void
 */
